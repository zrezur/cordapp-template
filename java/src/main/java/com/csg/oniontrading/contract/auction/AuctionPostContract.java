...
public class AuctionPostContract implements Contract {
    ...
    public interface Commands extends CommandData {
        class Post implements IssueCommand, TradingContract.Commands {
            private final long nonce = Utils.random63BitValue();
            @Override public long getNonce() { return nonce; }
        }

        class Buy implements IssueCommand, TradingContract.Commands{
            private final long nonce = Utils.random63BitValue();
            @Override public long getNonce() { return nonce; }
        }
    }
    ...
    public interface Clauses {
         ...
         */
        class Auction extends Clause<TradingState, TradingContract.Commands, UniqueIdentifier> {
            @Override public Set<TradingContract.Commands> verify(TransactionForContract tx,
...
//                            out.getPurchaseOrder().getDeliveryDate().toInstant().isAfter(time));
     ...
     */
    private final SecureHash legalContractReference = SecureHash.sha256("auction order contract template and params");
